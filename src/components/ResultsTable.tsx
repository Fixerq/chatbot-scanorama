  ...
  return <div className="rounded-md border">
      <Table>
        ...
        <TableBody>
          ...
          return <TableRow key={index} className={isLoading ? 'opacity-50' : ''}>
                <ResultUrlCell url={websiteUrl} />
                <TableCell className="font-medium">
                  {businessName}
                </TableCell>
                <ResultStatusCell status={formatAnalysisStatus(result.status, isResultAnalyzing)} hasChatbot={hasChatbot} technologies={technologies} lastChecked={result.lastChecked || result.details?.lastChecked} chatSolutions={result.chatbot_solutions || result.details?.chatSolutions} isAnalyzing={isResultAnalyzing} />
              </TableRow>;
        })}
        </TableBody>
      </Table>
