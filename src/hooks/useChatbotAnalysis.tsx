  ...
  const analyzeChatbots = async (results: Result[]): Promise<Result[]> => {
    ...
    try {
      const analyzedResults = await Promise.all(
        results.map(async (result) => {
          ...
          try {
            ...
            // Enhanced verification with very lenient confidence check
            const hasChatbot = response.chatSolutions && 
                             response.chatSolutions.length > 0 && 
                             !response.status?.toLowerCase().includes('no chatbot') &&
                             (response.confidence === undefined || response.confidence >= 0.3) && // Further lowered from 0.5
      ...
      const chatbotCount = analyzedResults.filter(r => 
        ...
        (r.details.confidence === undefined || r.details.confidence >= 0.3) // Further lowered from 0.5
      ).length;
